        ...
        <>
            ...
            <View className="mb-8">
                {devices.length === 0 ? (
                    ...
                    devices.map((item) => (
                        <Pressable key={item.BSSID} onPress={() => onToggleSelection(item.BSSID)}>
                            <ThemedView
                                className={`flex-row items-center p-5 mb-3 rounded-2xl ${
                                    selectedDevices[item.BSSID] ? "bg-indigo-500/10 border-indigo-500" : ""
                                }`}
                                bordered={!selectedDevices[item.BSSID]}
                            >
                                <View
